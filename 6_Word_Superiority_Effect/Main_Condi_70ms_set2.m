...
% ================== medium stimulus duration (70.6ms) ===========================
clear all;
clc;
...
for ii=0:2                              % word: 사용된 낱말
    for jj=0:5                          % position: 초중종, 초중종
        for kk=0:5                      % condition: word, non-word, letter
            rp(ii*36+jj*6+kk+1,   :)=[1 ii+3 jj kk 0 0 0 0 sd 0];
            rp(ii*36+jj*6+kk+109, :)=[1 ii+3 jj kk 0 1 0 0 sd 0];
            rp(ii*36+jj*6+kk+217, :)=[1 ii+3 jj kk 1 0 0 0 sd 0];
            rp(ii*36+jj*6+kk+325, :)=[1 ii+3 jj kk 1 1 0 0 sd 0];
        end
    end
end
...
for ii=1:432
    ...
    Screen(w, 'PutImage', fixation, loc);
    Screen('Flip', w);
    ready=GetChar;
